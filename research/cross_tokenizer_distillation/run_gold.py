...
"""Entry point for GOLD cross-tokenizer on-policy distillation.

Usage:
    uv run python run_gold.py --config configs/gold_math.yaml [overrides...]

Example:
    uv run python run_gold.py --config configs/gold_math.yaml \
        gold_distillation.max_num_steps=100 \
...
from cross_tokenizer_distillation.gold_algorithm import (
    GoldDistillMasterConfig,
    gold_distillation_train,
    setup,
)
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description="Run GOLD cross-tokenizer distillation training"
    )
    parser.add_argument(
        "--config",
        type=str,
...
def main() -> None:
    ...
    if not args.config:
        args.config = os.path.join(
            os.path.dirname(__file__), "configs", "gold_math.yaml"
        )
    ...
    config: GoldDistillMasterConfig = OmegaConf.to_container(config, resolve=True)
    ...
    # Setup data with student tokenizer
