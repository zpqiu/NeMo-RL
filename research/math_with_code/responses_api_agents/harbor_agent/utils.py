...
class HarborAgentUtils:
    ...
    def extract_input_from_trajectory(
        ...
        """Extract the initial system/user messages from an ATIF trajectory.

        Harbor tasks provide the instruction via a file (not through the NeMo Gym
        request body). We convert all context steps before the first agent turn
        into ``NeMoGymEasyInputMessage`` objects so the returned
        ...
        for step in trajectory.get("steps", []):
            ...
            if source in ("system", "user"):
                ...
                # Initial context always precedes the first agent step.
                break
    ...
    @staticmethod
    def validate_training_token_details(output_items: List[Dict[str, Any]]) -> None:
        """Validate the token contract required by NeMo-RL multi-turn rollouts."""
        assistant_messages = [
            item
        ...
        training_messages = [
            item for item in assistant_messages if "generation_token_ids" in item
        ]
        if not training_messages:
            raise ValueError("Harbor rollout contains no trainable generation token data")
        if len(training_messages) != len(assistant_messages):
            raise ValueError(
                "Harbor rollout is missing token details for one or more assistant turns"
            )

        seen_token_ids: List[int] = []
        for turn_index, item in enumerate(training_messages):
            ...
            if not isinstance(prompt_token_ids, list):
                raise ValueError(f"turn {turn_index} prompt_token_ids is not a list")
            if not isinstance(generation_token_ids, list) or not generation_token_ids:
                raise ValueError(f"turn {turn_index} generation_token_ids is empty")
            if not isinstance(generation_log_probs, list):
                raise ValueError(f"turn {turn_index} generation_log_probs is not a list")
            if len(generation_token_ids) != len(generation_log_probs):
                raise ValueError(
                    f"turn {turn_index} has {len(generation_token_ids)} generation "
                    f"tokens but {len(generation_log_probs)} logprobs"
                )
            if not all(
                isinstance(value, (int, float)) and math.isfinite(value)
                for value in generation_log_probs
            ):
                raise ValueError(f"turn {turn_index} contains non-finite logprobs")
            if seen_token_ids != prompt_token_ids[: len(seen_token_ids)]:
                raise ValueError(
                    f"turn {turn_index} token history is non-contiguous: "
                    f"seen={seen_token_ids}, prompt={prompt_token_ids}"
                )
